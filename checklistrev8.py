import streamlit as st
import os, base64
import pandas as pd
from datetime import datetime
from fpdf import FPDF

# Criar pasta de fotos se não existir
os.makedirs("fotos", exist_ok=True)

# Criar classe PDF
class PDF(FPDF):
    def header(self):
        logo_path = "temp_logo.jpeg"
        if not os.path.exists(logo_path):  # Salvar logotipo apenas uma vez
            with open(logo_path, "wb") as f:
                f.write(base64.b64decode(""))  # Substitua pelo código Base64 do seu logotipo
        self.image(logo_path, 10, 8, 33)
        self.set_font('Arial', 'B', 12)
        self.cell(0, 10, 'Checklist de Qualidade - WEG AGW 110-2.1 MW', 0, 1, 'C')
        self.set_font('Arial', '', 10)
        self.cell(0, 10, 'Operação e Manutenção', 0, 1, 'C')
        self.ln(10)
    
    def footer(self):
        self.set_y(-15)
        self.set_font('Arial', 'I', 8)
        self.cell(0, 10, f'Página {self.page_no()} de {{nb}}  |  Iqony Solutions do Brasil', 0, 0, 'C')

# Gerar PDF
def gerar_pdf(dados):
    pdf = PDF()
    pdf.alias_nb_pages()
    pdf.add_page()
    pdf.set_font("Arial", size=10)
    for chave, valor in dados["geral"].items():
        pdf.cell(0, 10, f"{chave}: {valor}", ln=True)
    pdf.ln(10)
    
    for sistema, detalhes in dados["sistemas"].items():
        pdf.add_page()
        pdf.set_font("Arial", 'B', 12)
        pdf.cell(0, 10, sistema, ln=True)
        pdf.ln(5)
        pdf.set_font("Arial", size=10)
        pdf.multi_cell(0, 10, f"Observações:\n{detalhes['observacoes']}")
        pdf.cell(0, 10, f"Avaliação: {detalhes['avaliacao']} / 5", ln=True)
        
        # Inserir imagens
        y_start = pdf.get_y()
        for i, foto in enumerate(detalhes['fotos'][:4]):
            pdf.image(foto, x=10 + (i % 2) * 100, y=y_start + (i // 2) * 65, w=90, h=60)
        pdf.ln(70)
    
    return pdf

# Criar estrelas clicáveis
def estrelas_clicaveis(label, key, max_stars=5):
    cols = st.columns(max_stars)
    for i in range(max_stars):
        if cols[i].button("⭐" if i < st.session_state.get(key, 3) else "☆", key=f"{key}_{i}"):
            st.session_state[key] = i + 1
    return st.session_state.get(key, 3)

# Interface Streamlit
st.title("Checklist de Qualidade - WEG AGW 110-2.1 MW")
st.header("Informações Gerais")
dados = {"geral": {chave: st.text_input(chave) for chave in ["Complexo Eólico", "Número da Turbina", "Inspetor", "Motivo da Parada"]}, "sistemas": {}}
dados["geral"]["Data da Inspeção"] = st.date_input("Data da Inspeção").strftime("%d/%m/%Y")

sistemas = ["Base", "Cubiculo MT", "Torre", "Elevador Principal", "Escada Vertical", "Linha de Vida", "YAW", "Transformador", "Conversores de Potência", "Estrutura da Nacele", "Guincho de Serviço", "Sensores Ambientais", "Sistema de Arrefecimento", "Sistema de Freio", "Gerador", "Pitch", "Sistema de Lubrificação", "Pás"]
for i, sistema in enumerate(sistemas):
    st.subheader(sistema)
    obs, stars = st.text_area(f"Observações", key=f"obs_{i}"), estrelas_clicaveis("Avaliação", key=f"stars_{i}")
    fotos = [os.path.join("fotos", foto.name) for foto in (st.file_uploader("Carregar fotos", accept_multiple_files=True, type=["jpg", "jpeg", "png"], key=f"fotos_{i}") or [])]
    for foto in fotos: open(foto, "wb").write(st.session_state[f"fotos_{i}"][0].read())
    dados["sistemas"][sistema] = {"observacoes": obs, "avaliacao": stars, "fotos": fotos}

# Gerar e baixar PDF
if st.button("Gerar PDF do Checklist"):
    pdf_output = "checklist_qualidade.pdf"
    gerar_pdf(dados).output(pdf_output)
    with open(pdf_output, "rb") as f:
        st.download_button("Baixar PDF", f, file_name="checklist_qualidade.pdf", mime="application/pdf")
